...
# Funções de Carregamento de Dados com Robustez Máxima
def load_data():
    ...
    if not os.path.exists("historico_jogadores.csv"):
        ...
        # RENOMEAÇÃO EXPLÍCITA E VERIFICAÇÃO
        rename_map = {
            "Data_Pelada": "Data_Evento", 
            "Status_Pagamento": "Status", 
            "Tipo_Evento": "Tipo_Participacao"
        }
        df_jogadores.rename(columns=rename_map, inplace=True)
        
        # Garantir que a coluna Data_Evento existe antes de converter
        if "Data_Evento" not in df_jogadores.columns:
            # Se nem a antiga nem a nova existem, cria uma coluna vazia
            df_jogadores["Data_Evento"] = pd.to_datetime("2024-01-01")
        
        # Conversão segura
        df_jogadores["Data_Evento"] = pd.to_datetime(df_jogadores["Data_Evento"], errors='coerce')
        
        # Garantir outras colunas
        ...
        if "Valor_Pago" not in df_jogadores.columns:
            df_jogadores["Valor_Pago"] = df_jogadores["Status"].apply(lambda x: 20.0 if str(x) == "Pago" else 0.0) if "Status" in df_jogadores.columns else 0.0
            
    # 3. Carregar Base de Jogadores
...
def match_player_name(input_name, base_names):
    ...
    clean_name = re.sub(r'\(.*?\)', '', str(input_name))
    ...
    if tipo_lanc == "Processar Listas (Semanal)":
        ...
        if st.button("Analisar Listas"):
            ...
            if st.button("Confirmar e Salvar no Caixa"):
                ...
                new_id = df_caixa["ID"].max() + 1 if not df_caixa.empty else 1
                dt_ev = pd.to_datetime(data_evento)
                lancamentos = [
                    {"ID": new_id, "Data": dt_ev, "Descricao": "Arrecadação Lista", "Categoria": "Pelada", "Tipo": "Entrada", "Valor": total_arrec},
                    {"ID": new_id+1, "Data": dt_ev, "Descricao": "Aluguel Quadra", "Categoria": "Fixo", "Tipo": "Saída", "Valor": aluguel}
                ]
                if custo_carne > 0:
                    lancamentos.append({"ID": new_id+2, "Data": dt_ev, "Descricao": "Custos Churrasco", "Categoria": "Churrasco", "Tipo": "Saída", "Valor": custo_carne})
